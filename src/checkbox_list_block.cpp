...
checkbox_list_block::checkbox_list_block(style_flags style_, std::string symbol_)
: grid_(make_grid(0, 2)), style_(style_), symbol_(std::move(symbol_))
{
}
...
checkbox_list_block& checkbox_list_block::add(bool enabled, block description)
{
    ...
    if (enabled)
    {
        grid_.set(grid_.rows() - 1, 0, text("[") << text(symbol_, style_) << text("] "));
    }
    else
    {
...
checkbox_list_block make_checkbox_list(style_flags style, std::string symbol)
{
    return checkbox_list_block(style, std::move(symbol));
}
